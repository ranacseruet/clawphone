...
async function _getCoreDeps() {
  ...
  // Resolve openclaw package root from the installed binary's package.json main
  const root = join(fileURLToPath(import.meta.resolve("openclaw")), "..", "..");
...
function _buildPrompt(userText, mode) {
  if (mode === "sms") {
    const instruction =
    ...
    return `SMS (Rana): ${userText}\n\n${instruction}`;
  }
  return `Phone call (Rana): ${userText}`;
}
 ...
 */
export async function openclawReply({ userText, mode = "voice", run = defaultRun, _api, _coreDeps }) {
  // ── Plugin path ────────────────────────────────────────────────────────
  if (_api) {
    ...
    try {
      const result = await deps.runEmbeddedPiAgent({
        ...
        agentDir,
        config:          cfg,
        prompt:          _buildPrompt(userText, mode),
        verboseLevel:    "off",
        timeoutMs,
        runId:           `${mode}:${Date.now()}`,
        lane:            mode,
