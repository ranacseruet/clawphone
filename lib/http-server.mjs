...
export async function createServer(config, api = null) {
  ...
  const _openclawReply = ({ userText, mode }) => openclawReply({ userText, mode, _api: api });
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (req.method === "POST" && u.pathname === "/speech") {
      ...
      // Log to Discord
      if (said) {
        void _discordLog({ text: `📞 **Phone (Rana)**: ${said}` }).catch((e) =>
      ...
      (async () => {
        ...
        if (reply) {
          void _discordLog({ text: `📞 **Tom**: ${reply}` }).catch((e) =>
            console.error(`[discordLog] ${String(e)}`)
          );
        }
